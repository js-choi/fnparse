...
(def number-level-expr
  (with-label "number"
    (alt (complex [first-digits #'number-level-expr, next-digit digit]
...
(def symbol-expr
  (alt (complex [first-char symbol-char, next-chars #'symbol-expr]
         (cons first-char next-chars))
       (semantics symbol-char list)))

(def symbol-level-expr
  (with-label "symbol" (semantics symbol-expr #(apply str %))))

(def terminal-level-expr
  (alt number-level-expr symbol-level-expr))
...
(def function-level-expr
  (alt (vconc symbol-level-expr parenthesized-expr) parenthesized-level-expr))
